...
// Marcos 2026-07-21: extraído del route `/conversations/[id]` para
// que la vista de detalle sea reutilizable dentro del split-pane del
// listado. La página `[id]` sigue funcionando como wrapper fino para
// deep-links (mensajes/email al operador). En modo `embedded`,
// omitimos el header con "volver" fullscreen — el layout padre del
// split ya provee su propia navegación.
...
export function ConversationDetailPanel({ conversationId, onBack, embedded }: ConversationDetailPanelProps) {
    ...
    <div className={"flex flex-col animate-in fade-in duration-500 gap-4 " + (embedded ? "h-full min-h-[500px] overflow-hidden" : "h-[calc(100dvh-140px)] min-h-[500px] overflow-hidden")}>
      ...
      <div className={"flex shrink-0 items-center justify-between gap-2 " + (embedded ? "hidden sm:flex" : "")}>
        <button
          type="button"
          onClick={() => (onBack ? onBack() : router.push("/conversations"))}
          className={"group h-10 shrink-0 items-center gap-1.5 rounded-full px-3 text-sm font-medium text-slate-600 transition-all duration-200 hover:bg-slate-100 hover:text-slate-900 active:scale-[0.97] " + (embedded ? "hidden" : "inline-flex")}
        >
          <ArrowBackIcon
            sx={{ fontSize: 16 }}
            className="transition-transform duration-300 group-hover:-translate-x-0.5"
